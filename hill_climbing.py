...
def hill_climbing(bot_type, bounds, max_iter=1000):
    ...
    for i in range(max_iter):
        ...
        if bot_type.lower() == "sma":
            new_high_frequency_window, new_low_frequency_window = window_tweak(
                high_window, low_window, bounds[0], bounds[1]
            )
        elif bot_type.lower() == "smaema":
            new_high_frequency_window, new_low_frequency_window = window_tweak(
                high_window, low_window, bounds[0], bounds[1]
            ...
            # generate the values
            new_high_frequency_window, new_low_frequency_window = complex_tweak(
                high_window, low_window, bounds
            )

        # run the total cash return after trading
        cash1 = bot_training.bot_training(bot_type, high_window, low_window, alpha)
        cash2 = bot_training.bot_training(
            bot_type, new_high_frequency_window, new_low_frequency_window, new_alpha
        )
    ...
    convergence_df = pd.DataFrame(
        {"No. of iteration": list(range(1, len(convergence) + 1)), "Cash": convergence}
    )
    if bot_type.lower() == "sma" or bot_type.lower() == "complex":
        convergence_df.to_csv(
            "{} hill_climbing_convergence.csv".format(bot_type.lower()), index=False
        )
        return [high_window, low_window], float(cash1)
    elif bot_type.lower() == "smaema":
        convergence_df.to_csv(
            "{} hill_climbing_convergence.csv".format("smaema"), index=False
        )
...
def window_tweak(hfw, lfw, window_range1, window_range2):
    ...
    for _ in range(100):
        ...
        new_hfw = max(
            window_range1[0], min(window_range1[1], new_hfw)
        )  # can change the bounds to 5-50
        new_lfw = max(
            window_range2[0], min(window_range2[1], new_lfw)
        )  # can change the bounds to 51-100
        if new_hfw < new_lfw:
